...
function AdminDashboard() {
  ...
  const handleAddPlanSubmit = async (e) => {
    ...
    try {
      if (editingPlanId) {
        await axios.put(`${API_BASE}/admin-role/membership-plan/update/${editingPlanId}`, planForm);
  ...
  const handleDeletePlan = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE}/admin-role/membership-plan/delete/${id}`);
      fetchTabData();
    } catch (err) {
      alert("Delete failed");
    }
      ...
      <main className="flex-1 p-4 sm:p-6 overflow-x-hidden">
                        ...
                        {activeTab === "plans" && (
                          <>
                            ...
                            <td className="px-3.5 py-2.5">
                              <div className="flex items-center gap-1.5">
                                <button className="px-2 py-1 rounded text-[11px] font-semibold cursor-pointer transition-colors bg-sky-600 hover:bg-sky-500 text-white border-none" onClick={() => setSelectedItemDetails(item)}>👁️ View Details</button>
                                <button className="px-2 py-1 rounded text-[11px] font-semibold cursor-pointer transition-colors bg-amber-600 hover:bg-amber-500 text-white border-none" onClick={() => handleEditPlan(item)}>✏️ Edit</button>
                                <button className="px-2 py-1 rounded text-[11px] font-semibold cursor-pointer transition-colors bg-red-600 hover:bg-red-500 text-white border-none" onClick={() => handleDeletePlan(item._id)}>Delete</button>
                              </div>
                            </td>
                          </>
                        )}
        ...
        {showModal && activeTab === "plans" && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl text-left">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-base font-bold text-white m-0">{editingPlanId ? "Edit Membership Plan" : "Add Membership Plan"}</h3>
                <button className="bg-transparent border-none text-slate-400 text-lg cursor-pointer hover:text-white" onClick={() => { setShowModal(false); setEditingPlanId(null); }}>✕</button>
              </div>
              <form onSubmit={handleAddPlanSubmit} className="space-y-3.5">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Assign to Gym</label>
                  <select className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-lg text-white text-xs outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 cursor-pointer" value={planForm.gymId} onChange={(e) => setPlanForm({ ...planForm, gymId: e.target.value })}>
                    <option value="">-- All Gyms (Global Plan) --</option>
                    {adminGyms.map((g) => (
                      <option key={g._id} value={g._id}>{g.gymName}</option>
                    ))}
                  </select>
                ...
                <div className="flex gap-2.5 justify-end pt-3">
                  <button type="button" className="bg-slate-800 border border-slate-700 text-slate-200 px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer hover:bg-slate-700 transition-colors" onClick={() => { setShowModal(false); setEditingPlanId(null); }}>Cancel</button>
                  <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-1.5 rounded-lg font-semibold text-xs transition-colors cursor-pointer border-none">{editingPlanId ? "Update Plan" : "Save Plan"}</button>
                </div>
              </form>
            </div>
          </div>
